...
class DatasetBase:
    ...
    def configure_test(self) -> torch.utils.data.Dataset:
        raise NotImplementedError
        
    def get_test(self) -> torch.utils.data.Dataset:
        if self._test_ds is None:
            self._test_ds = self.configure_test()
...
class DiabeticRetinopathyGradingDataset(DatasetBase):
    ...
    def configure_train(self):
        img_path = 'data/C. Diabetic Retinopathy Grading/train_data/train/Training_Set'
        csv_file_path = 'data/C. Diabetic Retinopathy Grading/2. Groundtruths/a. DRAC2022_ Diabetic Retinopathy Grading_Training Labels.csv'

        return ChallengeDataset(img_path, csv_file_path, transform=self.transform_train)

    def configure_validation(self):
        img_path = 'data/C. Diabetic Retinopathy Grading/train_data/val/Training_Set'
        csv_file_path = 'data/C. Diabetic Retinopathy Grading/2. Groundtruths/a. DRAC2022_ Diabetic Retinopathy Grading_Training Labels.csv'
    ...
    def get_train(self) -> torch.utils.data.Dataset:
        if self._train_ds is None:
            print("[DiabeticRetinopathyGradingDataset] self.train_ds is None")
            self._train_ds = self.configure_train()
        return self._train_ds
...
def get_moco_dataset(hparams: ModelParams) -> DatasetBase:
    if hparams.dataset_name == "stl10":
    ...
    elif hparams.dataset_name == "challenge":
        crop_size = 224
        resize = 256
        normalize_means = [0.485, 0.456, 0.406]
        ...
        return DiabeticRetinopathyGradingDataset(
            transform_train=transforms.split_transform, transform_test=transforms.get_test_transform()
        )
    else:
...
def get_encoder(name: str, dataset: str, **kwargs) -> torch.nn.Module:
    ...
    def set_parameter_requires_grad(model, feature_extract=True):
        if feature_extract:
            ...
            print("Froze the parameters")

    if name in ws_resnet.__dict__:
        print(f"our encode model name is {name}")
    ...
    if hasattr(model, "fc"):
        ...
        # if dataset == "cifar10":
        if dataset == "cifar10" or dataset=='challenge':
            print(f"model has fc,our encode model:{name} is in torchvision, dataset:{dataset}")
